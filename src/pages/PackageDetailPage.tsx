...
import type { ServiceTier } from "@/constants/packages";

const tierServiceIcons = [
  { key: "hotel" as keyof ServiceTier, label: "Hotel", Icon: Hotel },
  { key: "meals" as keyof ServiceTier, label: "Meals", Icon: Utensils },
  { key: "restaurant" as keyof ServiceTier, label: "Restaurant", Icon: UtensilsCrossed },
  { key: "transport" as keyof ServiceTier, label: "Transport", Icon: Car },
];

const PackageDetailPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const pkg = getPackageBySlug(slug ?? "");
  const [selectedTierStars, setSelectedTierStars] = useState<2 | 3 | 4 | 5>(3);
  ...
  const waMsg = encodeURIComponent(
    `Hi, I'm interested in the *${pkg.title}* . Please share availability and details.`,
  );
            ...
            <div className="lg:col-span-2 space-y-12">
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {pkg.highlights.map((h) => (
                    <div
                      key={h}
                      className="flex items-start gap-2.5 bg-accent/50 border border-border/50 rounded-xl px-4 py-3"
                    >
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-5">
                  Choose your preferred star category — applies to all services including hotels, meals, restaurants, and transport.
                </p>

                {/* Star selector */}
                <div className="flex flex-wrap gap-3 mb-6">
                  {pkg.serviceTiers.map((tier) => {
                    const active = selectedTierStars === tier.stars;
                    return (
                      <button
                        key={tier.stars}
                        onClick={() => setSelectedTierStars(tier.stars)}
                        className={`flex flex-col items-center gap-1 px-5 py-3 rounded-xl border-2 transition-all duration-200 min-w-[90px] ${
                          active
                            ? "border-secondary bg-secondary/10 shadow-md"
                            : "border-border/50 bg-card hover:border-secondary/50"
                        }`}
                      >
                        <div className="flex gap-0.5">
                          {Array.from({ length: tier.stars }).map((_, i) => (
                            <Star
                              key={i}
                              className={`w-3.5 h-3.5 ${active ? "fill-secondary text-secondary" : "fill-muted text-muted-foreground"}`}
                            />
                          ))}
                        </div>
                        <span className={`text-xs font-semibold ${active ? "text-secondary" : "text-muted-foreground"}`}>
                          {tier.label}
                        </span>
                      </button>
                    );
                ...
                {/* Tier details */}
                {(() => {
                  const tier = pkg.serviceTiers.find((t) => t.stars === selectedTierStars);
                  if (!tier) return null;
                  return (
                    <div className="grid sm:grid-cols-2 gap-4">
                      {tierServiceIcons.map(({ key, label, Icon }) => (
                        <div
                          key={key}
                          className="flex items-start gap-3 bg-accent/40 border border-border/50 rounded-xl p-4"
                        >
                          ...
                          <div>
                            ...
                            <p className="text-sm text-foreground">{String(tier[key])}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="bg-card border border-border/50 rounded-2xl shadow-lg overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  ...
                  <a href="tel:9682553190">
                    <Button
                      variant="outline"
                      className="w-full border-primary text-primary hover:bg-primary hover:text-primary-foreground gap-2 h-11"
                    >
